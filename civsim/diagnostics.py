...
Diagnostics and Historical Chronicler: Compiles active system metrics.
Exposes environmental trends, market metrics, progressive taxes, political types, and banking credit tracks.
"""
...
def report(world: World, agents: AgentRegistry, settlements: SettlementRegistry, ticks_run: int) -> dict:
    ...
    for s_id, s in settlements.settlements.items():
        ...
        info = {
            ...
            'regime': getattr(s, 'government_type', 'TRIBAL_CHIEFDOM'),
            'bank_status': getattr(s, 'central_bank_status', 'NONE'),
            'loans': getattr(s, 'outstanding_loans_count', 0),
            'bank_action': getattr(s, 'last_bank_action', 'NO_BANK_FOUNDED'),
            'defense_class': getattr(s, 'defense_tier', 'EARTHEN_DIKE'),
            'coup_milestone': getattr(s, 'last_political_milestone', 'NONE')
        }
